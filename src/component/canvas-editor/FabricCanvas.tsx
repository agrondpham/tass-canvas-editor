...
import {
  ...
  parseSelectedObjects,
} from "../common/FuncCopyParseDelete";
import {
  functionAddElementToCanvas,
  functionLoadFont,
  functionLoadJsonToCanvas,
} from "../common/FuncFabricRenderEleement";
import 'tass-fabric-history'
import { isMacOs } from "../../utils/helper";

export interface FabricCanvasRef {
  exportToJson: () => string;
}

export interface FabricCanvasCustom extends fabric.Canvas {
    undo: () => void
    redo: () => void
    clearHistory: () => void
}
  ...
  (
    ...
    useImperativeHandle(ref, () => ({
      exportToJson() {
        if (fabricCanvasRef.current) {
          const tempCanvas = fabricCanvasRef.current as fabric.Canvas;
          var json_data = JSON.stringify(tempCanvas.toDatalessJSON());
          return json_data;
        }
        return "";
      },
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (fabricCanvasRef.current) {
        ...
        } else if (((event.ctrlKey && !isMacOs()) || (event.metaKey && isMacOs())) &&
        (event.key === "z" || event.key === "Z")) {
            const tempCanvas = fabricCanvasRef.current as FabricCanvasCustom;
            tempCanvas?.undo()
    ...
    useEffect(() => {
      if (canvasRef.current) {
        ...
        functionLoadFont(canvasFonts, (fonts) => {
          ...
          //load first time
          functionLoadJsonToCanvas(fabricCanvasRef.current! as FabricCanvasCustom, initData, mode);
          setFinishLoad(true);
        });
